...
def solution():
    ...
    def flash_counter(grid = grid, steps = 100):
        # number of rows
        n = len(grid)
        # number of columns
        m = len(grid[0])
        ...
        for step in range(steps):
            ...
            for i in range(n):
                ...
                for j in range(m):
                    ...
                    if grid[i][j] > 9:
                        # we need to flash it and check what happens to its neighbors
                        step_flashes += flashes(grid, i, j, 0)

            flash_counter += step_flashes
            #print (f"step : {step}\n", np.array(grid))
        #print ("last step: \n", np.array(grid))
    ...
    def flashes(grid, i , j, flashed_position = 0):
        ...
        while not queue.empty():
            ...
            if i < 0 or i >= n or j < 0 or j >= m or grid[i][j] == 0 :
                continue
            # otherwise increase energy level:
            grid[i][j] += 1
            # if after increase, the current_position has a energy level > 9
            if grid[i][j] > 9:
