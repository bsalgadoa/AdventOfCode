...
def solution():
    ...
    def paths(node = "start", visited_nodes = set(), total_paths = 0):

        node_conections = set(nodes_dict[node])

        while node_conections:

            node_conections.add(node)
            node = node_conections.pop()
